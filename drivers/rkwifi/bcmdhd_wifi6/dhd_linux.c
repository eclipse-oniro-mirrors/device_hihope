...
static inline dhd_dev_priv_t * DHD_DEV_PRIV(struct net_device *dev)
{
	...
	if (GetHdfNetDeviceByLinuxInf(dev) == g_hdf_netdev)
		__tmp_dhd_netpriv = ((dhd_dev_priv_t *)g_hdf_netdev->mlPriv);
	else
		__tmp_dhd_netpriv = ((dhd_dev_priv_t *)DEV_PRIV(dev));
...
//#define DHD_DEV_PRIV(dev)       ((dhd_dev_priv_t *)GetHdfNetDeviceByLinuxInf(dev)->mlPriv)
...
//#define DHD_DEV_PRIV(dev)       ((dhd_dev_priv_t *)get_dhd_priv_data())
//#define DHD_DEV_INFO(dev)       (((dhd_dev_priv_t *)get_dhd_priv_data())->dhd)
//#define DHD_DEV_IFP(dev)        (((dhd_dev_priv_t *)get_dhd_priv_data())->ifp)
//#define DHD_DEV_IFIDX(dev)      (((dhd_dev_priv_t *)get_dhd_priv_data())->ifidx)
//#define DHD_DEV_LKUP(dev)		(((dhd_dev_priv_t *)get_dhd_priv_data())->lkup)
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (protocol == ETHER_TYPE_802_1X) {
			...
			if (netDevice && netDevice->netDeviceIf != NULL && netDevice->netDeviceIf->specialEtherTypeProcess != NULL) {
				...
				DHD_ERROR(("%s: send EAPOL pkt ret=%d\n", __FUNCTION__, ret));
	...
	uint8 *mac, uint8 bssidx, bool need_rtnl_lock, const char *dngl_name)
{
	...
	DHD_ERROR(("%s: bdh6: create netdevice %d\n", __FUNCTION__, create_pri_inf));
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
    ...
    DHD_ERROR(("%s: bdh6 net name =%s, %p, %p\n", __FUNCTION__, net->name, net->netdev_ops, &dhd_ops_virt));
	...
	if (0 == create_pri_inf) {
		create_pri_inf = 1;
		DHD_ERROR(("%s: for primary inf don't register netdev %d\n", __FUNCTION__, create_pri_inf));
        
        // Call linux register_netdev()
        err = NetDeviceAdd(g_hdf_netdev);
        DHD_ERROR(("%s:NetDeviceAdd ret=%d\n", __FUNCTION__, err));
	} else {
		if (need_rtnl_lock)
...
{
	dhd_info_t *dhd_info = *(dhd_info_t **)DHD_DEV_PRIV((struct net_device *)dev);
...
{
	dhd_info_t *dhd_info = *(dhd_info_t **)DHD_DEV_PRIV((struct net_device *)ndev);
	dhd_pub_t *dhdp = &dhd_info->pub;
