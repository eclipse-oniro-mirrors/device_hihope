...
//extern int32_t wl_cfg80211_scan(struct wiphy *wiphy, struct net_device *ndev,
...
fail:
    if (request->ie != NULL) {
        OsalMemFree((void *)request->ie);     //add void *
        request->ie = NULL;
    }
...
int32_t HdfStartScan(NetDevice *netDev, struct WlanScanRequest *scanParam)
{
    ...
    struct cfg80211_scan_request *request =
        (struct cfg80211_scan_request *)OsalMemCalloc(sizeof(struct cfg80211_scan_request) + sizeof(struct ieeee80211_channel *)*channelTotal);
...
int32_t HdfConnect(NetDevice *netDev, WlanConnectParams *param)
{
    ...
    cfg80211_params.auth_type = (hi_u8)param->authType;
    cfg80211_params.privacy = param->privacy;
    cfg80211_params.key_len = param->keyLen;
    cfg80211_params.key_idx = param->keyIdx;
    cfg80211_params.mfp = (hi_u8)param->mfp;

    HDF_LOGE("%s: connect ssid: %s", __func__, cfg80211_params.ssid);
...
//    return wl_cfg80211_disconnect(wiphy, ndev, reasonCode);
